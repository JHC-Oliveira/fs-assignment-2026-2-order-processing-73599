...
namespace SportsStore.Controllers
{
	...
	public class OrderController : Controller
	{
		...
		public async Task<IActionResult> Checkout(Order order)
		{
			...
			{
				_logger.LogWarning("Checkout validation failed for customer {CustomerName}", order.Name);
				return View(order);
			}
			...
			try
			{
				...
				var clientSecret = await _paymentService.CreatePaymentIntentAsync(_cart.ComputeTotalValue());
			...
			{
				_logger.LogError(ex, "Failed to create Stripe PaymentIntent for customer {CustomerName}", order.Name);
		...
		public async Task<IActionResult> PaymentResult(string paymentIntentId)
		{
			...
			try
			{
				...
				if (status == "succeeded")
				{
					...
					// Send to distributed pipeline
					var apiItems = _cart.Lines.Select(l => new OrderApiItem(
						(int)l.Product.ProductID!,  // cast long to int
						l.Product.Name ?? "",
						l.Quantity,
						l.Product.Price
					)).ToList();

					var distributedOrderId = await _orderApiClient.CheckoutAsync(
						Guid.NewGuid(),
						apiItems
					);

					if (distributedOrderId.HasValue)
					{
						TempData["DistributedOrderId"] = distributedOrderId.Value.ToString();
						_logger.LogInformation(
							"Order submitted to distributed pipeline: {OrderId}",
							distributedOrderId);
					}
